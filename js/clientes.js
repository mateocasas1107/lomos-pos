...
export function renderClientes(container, state) {
    ...
    btnExportar.onmouseout = () => btnExportar.style.transform = 'scale(1)';
    
    btnExportar.onclick = () => {
        if (!state.clientes || state.clientes.length === 0) {
            const divInfo = document.createElement('div');
            divInfo.style = 'position:fixed; top:20px; right:20px; background:#e74c3c; color:white; padding:15px 25px; border-radius:8px; z-index:10000; box-shadow:0 5px 15px rgba(0,0,0,0.5); font-weight:bold;';
            divInfo.innerText = '⚠️ No hay clientes registrados para exportar.';
            document.body.appendChild(divInfo);
            setTimeout(() => divInfo.remove(), 3500);
            return;
        }
