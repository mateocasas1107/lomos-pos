 ...
 * AHORA CON AUTO-COMPLETADO DE CLIENTES DESDE LA BASE DE DATOS.
 */
...
export function renderDomicilios(container, state) {
    ...
    // Asegurarnos de que el arreglo de clientes exista para evitar errores
    if (!state.clientes) {
        state.clientes = [];
    }
    ...
    window.abrirModalPedido = () => {
        ...
        modalOverlay.innerHTML = `
            ...
            ">
                ...
                <div style="display: flex; flex-direction: column; gap: 20px;">
                    <!-- Modificación: Teléfono y Nombre arriba con doble buscador -->
                    <div style="display: flex; gap: 15px; flex-wrap: wrap;">
                        <div style="flex: 1; min-width: 180px;">
                            <label style="font-size: 0.75rem; color: #888; font-weight: bold; text-transform: uppercase;">Teléfono (Buscador)</label>
                            <input type="text" id="dom-telefono" list="lista-telefonos-bd" placeholder="Busque por número..." style="width:100%; padding:13px 15px; margin-top:8px; background:#0a0a0a; border:1px solid #333; color:white; border-radius:8px; outline:none; font-size: 0.95rem; box-sizing: border-box; transition: 0.2s;">
                            
                            <!-- Lista oculta para el teléfono -->
                            <datalist id="lista-telefonos-bd">
                                ${state.clientes.filter(c => c.telefono).map(c => `<option value="${c.telefono}">${c.nombre}</option>`).join('')}
                            </datalist>
                        </div>
                        <div style="flex: 1.5; min-width: 200px;">
                            <label style="font-size: 0.75rem; color: #888; font-weight: bold; text-transform: uppercase;">Nombre del Cliente (Buscador)</label>
                            <input type="text" id="dom-cliente" list="lista-clientes-bd" placeholder="Escriba o seleccione un cliente..." style="width:100%; padding:13px 15px; margin-top:8px; background:#0a0a0a; border:1px solid #333; color:white; border-radius:8px; outline:none; font-size: 0.95rem; box-sizing: border-box; transition: 0.2s;">
                            
                            <!-- Lista oculta para el nombre -->
                            <datalist id="lista-clientes-bd">
        ...
        const inputCliente = document.getElementById('dom-cliente');
        const inputTelefono = document.getElementById('dom-telefono');
        const inputDireccion = document.getElementById('dom-direccion');
        
        // Ahora ponemos el foco primero en el teléfono
        inputTelefono.focus();

        // LÓGICA DE AUTOCOMPLETADO POR NOMBRE
        inputCliente.addEventListener('input', (e) => {
            const nombreSeleccionado = e.target.value.trim();
            const clienteEncontrado = state.clientes.find(c => c.nombre === nombreSeleccionado);
            
            if (clienteEncontrado) {
                if (!inputTelefono.value) inputTelefono.value = clienteEncontrado.telefono || '';
                inputDireccion.value = clienteEncontrado.ubicacion || '';
                
                inputTelefono.style.borderColor = '#2ecc71';
                inputDireccion.style.borderColor = '#2ecc71';
                setTimeout(() => {
                    inputTelefono.style.borderColor = '#333';
                    inputDireccion.style.borderColor = '#333';
                }, 800);
            }
        });

        // LÓGICA NUEVA: AUTOCOMPLETADO POR TELÉFONO
        inputTelefono.addEventListener('input', (e) => {
            const telefonoSeleccionado = e.target.value.trim();
            const clienteEncontrado = state.clientes.find(c => c.telefono && c.telefono.trim() === telefonoSeleccionado);
            
            if (clienteEncontrado) {
                if (!inputCliente.value) inputCliente.value = clienteEncontrado.nombre || '';
                inputDireccion.value = clienteEncontrado.ubicacion || '';
                
                inputCliente.style.borderColor = '#2ecc71';
                inputDireccion.style.borderColor = '#2ecc71';
                setTimeout(() => {
                    inputCliente.style.borderColor = '#333';
                    inputDireccion.style.borderColor = '#333';
                }, 800);
            }
        });
        ...
        inputs.forEach(input => {
            input.addEventListener('focus', () => { if(input.style.borderColor !== 'rgb(46, 204, 113)') input.style.borderColor = '#e74c3c'; });
    ...
    window.cambiarEstadoPedido = (id) => {
        ...
        if (d) {
            // Ciclo de estados: Pendiente -> Enviado -> Entregado -> Pendiente
                    ...
                    <tbody>
                        ${domiciliosOrdenados.length > 0 ? domiciliosOrdenados.map(d => {
                            ...
                            if (d.estado === 'Entregado') {
                                tagStyle = 'background: rgba(46, 204, 113, 0.2); color: #2ecc71; border: 1px solid rgba(46, 204, 113, 0.5);';
                                btnText = 'Reabrir';
                                btnBg = '#7f8c8d';
                            } else if (d.estado === 'Enviado') {
                                tagStyle = 'background: rgba(52, 152, 219, 0.2); color: #3498db; border: 1px solid rgba(52, 152, 219, 0.5);';
                                btnText = '✔ Entregado';
                                btnBg = '#2ecc71';
                            } else { // Pendiente por defecto
                                tagStyle = 'background: rgba(241, 196, 15, 0.2); color: #f1c40f; border: 1px solid rgba(241, 196, 15, 0.5);';
                                btnText = '🛵 Enviar';
                                btnBg = '#3498db';
                            }
                                ...
                                <tr style="border-bottom: 1px solid #222;">
                                    ...
                                    <td style="padding: 15px; text-align: center;">
                                        <div style="display: flex; gap: 8px; justify-content: center;">
                                            <button onclick="window.cambiarEstadoPedido(${d.id})" class="btn-edit-small" style="background: ${btnBg}; color: white; border: none; padding: 8px 12px; font-weight: bold; border-radius: 6px; cursor: pointer;">
                                                ${btnText}
                                            </button>
                                            <button onclick="window.eliminarPedido(${d.id})" class="btn-delete-small" style="background: transparent; border: 1px solid #e74c3c; color: #e74c3c; padding: 8px 12px; border-radius: 6px; font-weight: bold; cursor: pointer;" title="Eliminar registro">
                                                ✕
                                            </button>
                                        </div>
                                    </td>
