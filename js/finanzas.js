 ...
 * INCLUYE: Cierre y resumen mensual de ventas.
 */
...
export function renderFinanzas(container, state) {
    ...
    const ventasContadoTurno = ventasTurno.filter(v => v.metodoPago === 'contado_efectivo' || v.metodoPago === 'contado_tarjeta').reduce((acc, v) => acc + v.total, 0); // Ajuste: contar ambos contados
    ...
    // --- CÁLCULO DE CIERRES MENSUALES ---
    const agruparVentasPorMes = () => {
        const meses = {};
        ...
        (state.ventasRealizadas || []).forEach(v => {
            ...
            // Crear una clave única (ej: "2026-08") para agrupar
            const clave = `${ano}-${String(mesNum + 1).padStart(2, '0')}`;
            const nombreMes = `${nombresMeses[mesNum]} ${ano}`;
            ...
            if (!meses[clave]) {
                meses[clave] = {
                    ...
                    costos: 0,
                    utilidad: 0,
                    productos: {} // Nuevo: Acumulador de productos para el ranking
                };
            }

            // Sumar los valores de cada factura al mes correspondiente
            meses[clave].cantidad += 1;
            meses[clave].ingresos += (v.total || 0);
            meses[clave].costos += (v.costo || 0);
            meses[clave].utilidad += (v.utilidad || 0);

            // Nuevo: Acumular la cantidad vendida de cada producto en la factura
            if (v.items) {
                v.items.forEach(item => {
                    const nombreProd = item.nombre || 'Desconocido';
                    // Tratamos de obtener la cantidad de las propiedades comunes usadas
                    const cant = parseFloat(item.cantidadVendida || item.cantidadDetalle || item.cantidad || 0);
                    if (!meses[clave].productos[nombreProd]) {
                        meses[clave].productos[nombreProd] = 0;
                    }
                    meses[clave].productos[nombreProd] += cant;
                });
            }
        });

        // Convertir el objeto de meses en un array y procesar el ranking de productos
        const mesesArray = Object.values(meses).map(mes => {
            // Transformar el objeto de productos en un array ordenado
            const topProductos = Object.entries(mes.productos)
                .map(([nombre, cantidad]) => ({ nombre, cantidad }))
                .sort((a, b) => b.cantidad - a.cantidad) // Orden descendente (los más vendidos primero)
                .slice(0, 5); // Tomar solo el Top 5 para no saturar la tabla
            
            return { ...mes, topProductos };
        });

        // Ordenar de más reciente a más antiguo (ej: Septiembre arriba, Agosto abajo)
        return mesesArray.sort((a, b) => b.clave.localeCompare(a.clave));
    };

    const ventasPorMes = agruparVentasPorMes();

    // --- GENERACIÓN DE EXCEL MENSUAL ---
    window.exportarMesExcel = (claveMes, nombreMes) => {
        // Filtrar todas las ventas que correspondan al mes seleccionado
        const ventasDelMes = (state.ventasRealizadas || []).filter(v => (v.fechaISO || "").startsWith(claveMes));
        
        if (ventasDelMes.length === 0) {
            mostrarAlertaModal("No hay ventas registradas para este mes.");
            return;
        }

        // Construir la estructura de la tabla de Excel
        let tablaHTML = `
            <html xmlns:o="urn:schemas-microsoft-com:office:office" xmlns:x="urn:schemas-microsoft-com:office:excel" xmlns="http://www.w3.org/TR/REC-html40">
            <head><meta charset="UTF-8"></head>
            <body>
                <h2>Reporte de Ventas Detallado - ${nombreMes}</h2>
                <table border="1">
                    <thead>
                        <tr>
                            <th style="background-color: #3498db; color: white; font-weight: bold;">N° Factura</th>
                            <th style="background-color: #3498db; color: white; font-weight: bold;">Fecha y Hora</th>
                            <th style="background-color: #3498db; color: white; font-weight: bold;">Cliente</th>
                            <th style="background-color: #3498db; color: white; font-weight: bold;">Método Pago</th>
                            <th style="background-color: #3498db; color: white; font-weight: bold;">Productos Vendidos</th>
                            <th style="background-color: #3498db; color: white; font-weight: bold;">Costo Mercancía ($)</th>
                            <th style="background-color: #3498db; color: white; font-weight: bold;">Ingreso Venta ($)</th>
                            <th style="background-color: #3498db; color: white; font-weight: bold;">Utilidad Neta ($)</th>
                        </tr>
                    </thead>
                    <tbody>
        `;
        ...
        ventasDelMes.forEach(v => {
            const cliente = v.cliente ? v.cliente.replace(/,/g, " ") : "Consumidor Final";
            const metodo = v.metodoPago === 'credito' ? 'Fiado (Crédito)' : 'Contado';
            // Agrupar los nombres de los productos para la celda de Excel
            const items = (v.items || []).map(i => `${i.cantidadDetalle} ${i.nombre}`).join(" | ");
            
            totalCosto += v.costo || 0;
            totalVenta += v.total || 0;
            totalUtilidad += v.utilidad || 0;

            tablaHTML += `
                <tr>
                    <td style="text-align: center;">${v.nroFactura || v.id}</td>
                    <td>${v.fecha}</td>
                    <td>${cliente}</td>
                    <td style="text-align: center;">${metodo}</td>
                    <td>${items}</td>
                    <td style="text-align: right;">${v.costo || 0}</td>
                    <td style="text-align: right;">${v.total || 0}</td>
                    <td style="text-align: right;">${v.utilidad || 0}</td>
                </tr>
            `;
        });
        ...
        tablaHTML += `
                    ...
                    <tfoot>
                        <tr>
                            <th colspan="5" style="text-align: right; font-weight: bold;">TOTALES DEL MES:</th>
                            <th style="text-align: right; font-weight: bold;">${totalCosto}</th>
                            <th style="text-align: right; font-weight: bold;">${totalVenta}</th>
                            <th style="text-align: right; font-weight: bold;">${totalUtilidad}</th>
                        </tr>
                    </tfoot>
                </table>
            </body>
        ...
        link.setAttribute("download", `Ventas_Lomos_${nombreMes.replace(/ /g, "_")}.xls`);
    ...
    <button class="btn-liquidar" data-cliente="${cliente}" style="background: #2ecc71; color: #000; border: none; padding: 8px 12px; border-radius: 6px; font-weight: bold; cursor: pointer;">
        Liquidar Todo
            ...
            <!-- SECCIÓN: CIERRES MENSUALES DE VENTAS -->
            <div class="table-container" style="margin-bottom: 35px; border-color: #3498db;">
                <div style="padding: 20px; border-bottom: 1px solid #2a2a2a; display: flex; justify-content: space-between; align-items: center; background: #1c1c13;">
                    <h3 style="margin: 0; color: #3498db;">📅 Cierres Mensuales de Ventas</h3>
                    <span style="color: #ccc; font-size: 0.85rem; background: #333; padding: 4px 10px; border-radius: 5px;">
                        Meses registrados: <b>${ventasPorMes.length}</b>
                    </span>
                </div>
                <table>
                    <thead>
                        <tr style="background: #1a1a1a;">
                            <th style="padding: 15px; text-align: left; color: #888; font-size: 0.75rem;">Mes / Año</th>
                            <th style="padding: 15px; text-align: left; color: #888; font-size: 0.75rem;">Top 5 Más Vendidos (Cant)</th>
                            <th style="padding: 15px; text-align: center; color: #888; font-size: 0.75rem;">Total Transacciones</th>
                            <th style="padding: 15px; text-align: right; color: #888; font-size: 0.75rem;">Ingresos Brutos</th>
                            <th style="padding: 15px; text-align: right; color: #888; font-size: 0.75rem;">Costo de Mercancía</th>
                            <th style="padding: 15px; text-align: right; color: #888; font-size: 0.75rem;">Utilidad del Mes</th>
                            <th style="padding: 15px; text-align: center; color: #888; font-size: 0.75rem;">Exportar</th>
                        </tr>
                    </thead>
                    <tbody>
                        ${ventasPorMes.length > 0 ? ventasPorMes.map(mes => `
                            <tr style="border-bottom: 1px solid #222;">
                                <td style="padding: 15px; font-weight: bold; color: #fff; font-size: 1rem; text-transform: capitalize; vertical-align: middle;">
                                    🗓️ ${mes.nombre}
                                </td>
                                <td style="padding: 15px; vertical-align: middle;">
                                    <div style="background: #111; padding: 8px 12px; border-radius: 6px; border: 1px solid #222; min-width: 180px; max-width: 250px;">
                                        ${mes.topProductos.length > 0 ? 
                                            mes.topProductos.map((p, i) => `
                                                <div style="display: flex; justify-content: space-between; align-items: center; border-bottom: ${i < mes.topProductos.length - 1 ? '1px dashed #333' : 'none'}; padding: 4px 0; gap: 10px;">
                                                    <span style="color: #ccc; font-size: 0.78rem; white-space: nowrap; overflow: hidden; text-overflow: ellipsis;" title="${p.nombre}">
                                                        <span style="color: #888; margin-right: 5px;">${i+1}.</span>${p.nombre}
                                                    </span>
                                                    <span style="color: #f1c40f; font-weight: bold; font-size: 0.85rem;">${p.cantidad}</span>
                                                </div>
                                            `).join('') 
                                            : '<div style="color: #555; font-size: 0.8rem; text-align: center;">Sin datos</div>'
                                        }
                                    </div>
                                </td>
                                <td style="padding: 15px; text-align: center; color: #ccc; vertical-align: middle;">
                                    ${mes.cantidad} facturas
                                </td>
                                <td style="padding: 15px; text-align: right; color: #2ecc71; font-weight: bold; vertical-align: middle;">
                                    $${mes.ingresos.toLocaleString()}
                                </td>
                                <td style="padding: 15px; text-align: right; color: #e74c3c; vertical-align: middle;">
                                    -$${mes.costos.toLocaleString()}
                                </td>
                                <td style="padding: 15px; text-align: right; color: #3498db; font-weight: bold; font-size: 1.1rem; vertical-align: middle;">
                                    $${mes.utilidad.toLocaleString()}
                                </td>
                                <td style="padding: 15px; text-align: center; vertical-align: middle;">
                                    <button onclick="window.exportarMesExcel('${mes.clave}', '${mes.nombre}')" 
                                            class="btn-primary" style="background: #27ae60; border: none; padding: 8px 12px; border-radius: 6px; font-weight: bold; cursor: pointer; font-size: 0.85rem;" title="Descargar Reporte Excel">
                                        📥 Excel
                                    </button>
                                </td>
                            </tr>
                        `).join('') : `
                            <tr>
                                <td colspan="7" style="text-align: center; padding: 40px; color: #555;">
                                    Aún no hay registros suficientes para generar un cierre mensual.
                                </td>
                            </tr>
                        `}
                    </tbody>
    ...
    // --- LÓGICA DE EVENTOS PARA BOTONES DE FIADOS ---
    // Nos aseguramos de añadir el listener solo una vez por renderizado (limpiando previos si los hubiera para evitar duplicados, aunque sobreescribir innerHTML suele ser suficiente, usar onlick en el contenedor es más seguro)
    container.onclick = (e) => {
...
// --- FUNCIONES GLOBALES PARA FIADOS (Fuera de renderFinanzas para evitar re-declaraciones) ---
