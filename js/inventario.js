 ...
 * Incluye gestión de Proveedores/Marcas y Código de Barras Opcional.
 */
...
export function renderInventario(container, state) {
    ...
    /**
     * MODAL PREMIUM DE CREACIÓN Y EDICIÓN DE PRODUCTO CON CÓDIGO DE BARRAS
     */
    window.showProductModal = (productToEdit = null) => {
        ...
        modalOverlay.innerHTML = `
            <div style="background: #141414; border: 1px solid #2a2a2a; border-radius: 16px; width: 100%; max-width: 500px; padding: 25px; box-shadow: 0 20px 40px rgba(0,0,0,0.7); color: #fff; box-sizing: border-box; display: flex; flex-direction: column; max-height: 95vh; overflow-y: auto;">
                ...
                <div style="display: flex; flex-direction: column; gap: 15px; flex-grow: 1;">
                    ...
                    <!-- CAMPO NUEVO: CÓDIGO DE BARRAS -->
                    <div>
                        <label style="font-size: 0.8rem; color: #3498db; font-weight: bold; text-transform: uppercase;">Barcode / Código de Barras (Opcional)</label>
                        <input type="text" id="m-codigoBarras" value="${isEditing ? (productToEdit.codigoBarras || '') : ''}" placeholder="Escanee o escriba el código..." style="width:100%; padding:11px 14px; margin-top:6px; background:#0a0a0a; border:1px solid #3498db66; color:white; border-radius:8px; outline:none; font-size: 0.95rem; box-sizing:border-box;">
                    </div>
                    ...
                    <div style="display: flex; gap: 12px;">
                        ...
                        <div style="flex: 1;">
                            ...
                            <input type="text" id="m-proveedor" value="${isEditing ? (productToEdit.proveedor || '') : ''}" placeholder="Ej: Alpina..." style="width:100%; padding:11px 14px; margin-top:6px; background:#0a0a0a; border:1px solid #333; color:white; border-radius:8px; outline:none; font-size: 0.95rem; box-sizing:border-box;">
                        </div>
                    </div>
                    ...
                    <div style="display: flex; gap: 10px;">
                        ...
                        <div style="flex: 1;">
                            <label style="font-size: 0.7rem; color: #9b59b6; font-weight: bold; text-transform: uppercase;">Editable</label>
                            <input type="number" id="m-precioAlto" value="${isEditing && productToEdit.precioAlto ? productToEdit.precioAlto : ''}" placeholder="Base" style="width:100%; padding:11px 10px; margin-top:6px; background:#0a0a0a; border:1px solid #9b59b666; color:white; border-radius:8px; outline:none; font-size: 0.9rem; box-sizing:border-box;">
                        </div>
                    </div>
                    ...
                    <div style="display: flex; gap: 12px; align-items: flex-end;">
                        ...
                        <div style="flex: 1;">
                            ...
                            <input type="number" id="m-stock" value="${isEditing ? productToEdit.stock : ''}" placeholder="Cantidad" style="width:100%; padding:11px 14px; margin-top:6px; background:#0a0a0a; border:1px solid #333; color:white; border-radius:8px; outline:none; font-size: 0.95rem; box-sizing:border-box;">
                        </div>
                    </div>
        ...
        setTimeout(() => {
            ...
            document.getElementById('m-btn-guardar').onclick = () => {
                ...
                const codigoBarras = document.getElementById('m-codigoBarras').value.trim(); // NUEVO CAMPO
                ...
                if (isEditing) {
                    ...
                    productToEdit.codigoBarras = codigoBarras; // Guardar código de barras
                    productToEdit.categoria = categoria;
                    productToEdit.proveedor = proveedor;
                    ...
                    const nuevo = {
                        id: Date.now(),
                        nombre, codigoBarras, categoria, proveedor, costo, precioDetal, precioMayor, precioAlto, stockMinimo, unidad, stock,
    ...
    const productosFiltrados = [...(state.productos || [])].filter(p => {
        ...
        const coincide = p.nombre.toLowerCase().includes(busq) || 
                         ...
                         (p.codigoBarras && p.codigoBarras.toLowerCase().includes(busq));
        ...
        <div class="module-fade-in">
            ...
            <div style="background: #141414; border: 1px solid #2a2a2a; border-radius: 12px; padding: 20px; margin-bottom: 25px; display: flex; gap: 15px; flex-wrap: wrap;">
                <div style="flex: 2; min-width: 250px;">
                    <input type="text" id="inv-search-input" placeholder="🔍 Buscar por nombre, categoría, proveedor o código de barras..." value="${state.inventarioBusqueda}" oninput="window.manejarBusquedaInventario(event)" style="width: 100%; padding: 12px 15px; border-radius: 8px; border: 1px solid #333; background: #0a0a0a; color: white; outline: none; font-size: 0.95rem;">
                </div>
                ...
                <table>
                    <thead>
                        <tr style="background: #1c1c1c;">
                            <th style="padding: 15px; text-align: left; color: #888;">Producto / Barcode</th>
                            ...
                            <th style="padding: 15px; text-align: right; color: #888;">Precios (Det/May/Edit)</th>
                    ...
                    <tbody>
                        ${productosFiltrados.length > 0 ? productosFiltrados.map(p => {
                            ...
                            if (p.lotes && p.lotes.length > 0) {
                                vencimientoTag = `<span class="tag" style="background:#2ecc71; color:black; font-weight:bold;">📋 Multi-Lote Ok</span>`;
                            } else if (p.fechaCaducidad) {
                                vencimientoTag = `<span class="tag" style="background:#2ecc71; color:black; font-weight:bold;">✅ Ok</span>`;
                            }
                            ...
                            return `
                                <tr style="border-bottom: 1px solid #222;">
                                    <td style="padding: 15px;">
                                        ...
                                        <div style="display:flex; gap:10px; margin-top:4px; font-size: 0.75rem;">
                                            ${p.proveedor ? `<span style="color: #888;">🏢 ${p.proveedor}</span>` : ''}
                                            ${p.codigoBarras ? `<span style="color: #3498db; font-family: monospace; background:#0d1b2a; padding:1px 6px; border-radius:4px;">🏷️ ${p.codigoBarras}</span>` : ''}
                                        </div>
                                    </td>
                                    ...
                                    <td style="padding: 15px; text-align: right;">
                                        ...
                                        ${p.precioAlto ? `<div style="color: #9b59b6; font-size: 0.75rem;">E: $${p.precioAlto.toLocaleString()}</div>` : ''}
                                    </td>
